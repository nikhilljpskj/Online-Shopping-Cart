<div class="container content-invoice">
    <div class="cards">
        <div class="card-bodys">
            <form action="<?php echo base_url('dashboard/update_purchase'); ?>" id="invoice-form" method="post" class="invoice-form" role="form" novalidate="">
               <input type="hidden" name="update_id" value="<?php echo $postData;?>" />
                            ...
                            <div class="form-group">
                                <select class="form-control form-control-lg" name="companyName" id="companyName" required="required">
                                            ...
                                            <?php foreach ($supplier as $key => $vals) { ?>
                                                <option data-supplier_name="<?php echo ($vals['supplier_name']); ?>" data-supplieraddress="<?php echo ($vals['supplier_primaryaddress']); ?>" data-supplier_areaprimary="<?php echo ($vals['supplier_areaprimary']); ?>" data-supplier_districtprimary="<?php echo ($vals['supplier_districtprimary']); ?>" data-supplier_stateprimary="<?php echo ($vals['supplier_stateprimary']); ?>" data-supplier_pincodeprimary="<?php echo ($vals['supplier_pincodeprimary']); ?>" data-supplier_gst="<?php echo ($vals['supplier_gst']); ?>" data-supplier_contactno1="<?php echo ($vals['supplier_contactno1']); ?>" value="<?php echo $vals['id']; ?>"><?php echo $vals['supplier_name']; ?></option>
                                            <?php } ?>
                                        </select>
                                ...
                                <tr>
                                    <td><select class="productsList form-control form-control-lg" name="productName[]" id="productName_0" required="required">
                                            ...
                                            <?php foreach ($productdetails as $key => $vals) { ?>
                                                <option data-id="0" data-category="<?php echo $vals['board']; ?>" value="<?php echo $vals['id']; ?>" <?php if($invoicedatadetails[0]['item_code'] ==$vals['id']){echo "selected";} ?>><?php
                                                if($invoicedatadetails[0]['item_code'] ==$vals['id']){
                                                    $catname = $vals['board']; 
                                                }
                                                 echo $vals['material']; ?></option>
                                            <?php } ?>
                                        </select>
<!--                                        <input type="text" name="productName[]" id="productName_1" class="form-control" autocomplete="off">-->
                                    </td>
                                    <td><input type="text" name="category[]" value="<?php echo $catname;?>" id="category_0" data-id="0" class="form-control" autocomplete="off" readonly></td>
                                    <td><input type="number" name="quantity[]" value="<?php echo $invoicedatadetails[0]['order_item_quantity'];?>" readonly='readonly' id="quantity_0" data-id="0" class="form-control quantity" autocomplete="off"></td>
